...
function processFile(filePath) {
  ...
  // We look for any of these specific classes that are on buttons and replace them with admin-cancel-btn
  targetClasses.forEach(cls => {
    // We only replace if they are used near a Cancel button, but it's safe to just replace them
    // since they are extremely specific long class strings used on these cancel buttons
    content = content.replace(new RegExp(cls.replace(/\[/g, '\\\\[').replace(/\]/g, '\\\\]'), 'g'), 'className="admin-cancel-btn"');
  });
...
const filesToProcess = [
  'BulkOrdersAdminPage.jsx',
  'OrdersPage.jsx',
  'cancellations/CancellationManagementPage.jsx',
  'catalog/CategoriesPage.jsx',
  'catalog/SubCategoriesPage.jsx',
  'refunds/RefundManagementPage.jsx',
  'reviews/ReviewManagementPage.jsx',
