        ...
        {['Customer Details', 'Shipping Address', 'Product Configuration', 'Upload & Notes', 'Submit Request'].map((step, idx) => {
          ...
          return (
            <React.Fragment key={step}>
              <div ref={isActive ? activeStepRef : null} className={`flex items-center gap-2 transition-opacity duration-300 ${isActive || isCompleted ? 'opacity-100' : 'opacity-50'}`}>
                <span className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors duration-300 ${isActive || isCompleted ? 'bg-[#9E6544] text-white border border-[#9E6544]' : 'border border-[#9E6544] text-[#9E6544] bg-transparent'}`}>
                  {isCompleted ? <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg> : stepNum}
                </span>
                {step}
              </div>
              {idx < 4 && <div className={`border-t-2 w-12 lg:w-20 transition-colors duration-300 ${isCompleted ? 'border-[#9E6544]' : 'border-dashed border-[#D2C5BB]'}`}></div>}
            </React.Fragment>
          );
        })}
      </div>

      <form onSubmit={handleSubmit} noValidate className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 pb-12">
        ...
        {currentStep === 5 && (
          <div className="bg-[#FAF8F5] rounded-2xl shadow-sm border border-[#E9DED3] p-10 text-center space-y-8">
            <div>
              <h2 className="text-2xl font-bold text-[#7A4B3A] mb-3">Almost Done!</h2>
              <p className="text-gray-600 text-sm max-w-lg mx-auto">Please review your details. Once submitted, our team will review your requirements and get back to you with a quote within 24-48 hours.</p>
            </div>
            
            <label className="flex items-center justify-center gap-3 cursor-pointer p-4 bg-white border border-[#E9DED3] rounded-xl max-w-md mx-auto">
              <input type="checkbox" required checked={formData.agreed} onChange={(e) => handleChange(null, 'agreed', e.target.checked)} className="w-5 h-5 text-[#9E6544] rounded border-gray-300 focus:ring-[#9E6544]" />
              <span className="text-sm font-semibold text-gray-700">I confirm the details and agree to the <span className="text-[#9E6544]">terms & conditions</span>.</span>
            </label>
            
            <button type="submit" disabled={loading} className="px-12 py-4 bg-[#9E6544] hover:bg-[#7A4B3A] text-white font-bold tracking-wider rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 mx-auto disabled:opacity-70 disabled:cursor-not-allowed">
        ...
        <div className="flex items-center justify-between pt-8 border-t border-[#E9DED3]">
          {currentStep > 1 ? (
             <button type="button" onClick={handlePrevStep} className="px-6 py-2.5 border border-[#9E6544] text-[#9E6544] font-semibold rounded-lg hover:bg-[#9E6544] hover:text-white transition-colors">
               Back
             </button>
          ) : <div></div>}

          {currentStep < 5 && (
             <button type="button" onClick={handleNextStep} className="px-8 py-2.5 bg-[#9E6544] text-white font-semibold rounded-lg hover:bg-[#7A4B3A] transition-colors shadow-sm">
               Continue
             </button>
          )}
        </div>
