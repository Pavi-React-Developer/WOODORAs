      ...
      <div className={`flex flex-col flex-1 bg-white ${viewMode === 'list' ? 'p-4 sm:p-8 justify-center gap-2' : 'p-4'}`}>
        ...
        {product.reviewCount > 0 && (
          <div className="flex items-center gap-1 mt-auto">
            ...
            <span className={`font-medium text-[#666666] ${viewMode === 'list' ? 'text-sm sm:text-base' : 'text-[11px]'}`}>{product.averageRating} <span className="text-[#999999] font-normal">({product.reviewCount})</span></span>
          </div>
        )}
      </div>
    </motion.div>
