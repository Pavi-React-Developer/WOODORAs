  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (const item of items) {
        ...
        // Product Details
        doc.font('Helvetica-Bold').fontSize(10).fillColor(colors.textDark).text(item.name, 160, y + 30, { width: 170 });
      ...
      const calculatedItemsGst = Array.isArray(order.orderItems) 
        ...
        : 0;
      const gstAmt = Number(order.gstAmount) || Number(order.totalGst) || calculatedItemsGst || 0;
      console.log(`[Invoice PDF Debug] Order ID: ${order._id}, gstAmt calculated as: ${gstAmt}`);
      if (gstAmt > 0) rowCount++;
      ...
      if (gstAmt > 0) drawPaymentRow('GST Amount', `+${gstAmt.toLocaleString('en-IN')}`);
